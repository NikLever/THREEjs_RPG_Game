class Game{
		...
		if (this.onAction.mode !== undefined){
			switch(this.onAction.mode){
                ...
                case 'collect':
                    ...
                    document.getElementById("briefcase").children[0].children[this.onAction.index].children[0].children[0].src = this.onAction.src;
    ...
    loadKey(loader){
		...
		loader.load( `${this.assetsPath}fbx/usb.fbx`, function ( object ) {
			...
			object.scale.set(scale, scale, scale);
			object.name = "key";
            object.position.set(-416, 0.8, -572);
            object.castShadow = true;
	...
	animate() {
        ...
        if (this.collect !== undefined && !trigger){
            ...
            this.collect.forEach(function(object){
				if (object.visible && game.player.object.position.distanceTo(object.position)<100 && !trigger){
					game.actionBtn.style = 'display:block;';
					game.onAction = { 
